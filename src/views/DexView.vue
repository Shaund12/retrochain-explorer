...
const activeTab = ref<"swap" | "liquidity" | "charts">("swap");
...
const balanceOf = (denom?: string) => {
  if (!denom) return "0";
  const map = userBalances.value || {};
  return map[denom] || "0";
};
...
const selectedLpBalanceAtomic = computed(() => (selectedLpDenom.value ? balanceOf(selectedLpDenom.value) : "0"));
...
<template>
  <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-6 overflow-hidden">
    ...
    <div class="flex items-center gap-2 mt-2">
      ...
      <button
        class="btn-secondary px-3 py-2"
        :class="activeTab === 'charts' ? 'ring-2 ring-indigo-400/60 text-indigo-200' : ''"
    ...
    <div v-else-if="activeTab === 'liquidity'" class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div class="lg:col-span-2 space-y-4">
